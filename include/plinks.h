...
* they violate the constarints of their link. It is used as a
* base class for cables and rods, and could be used as a bas
* class fo springs with limit to their extensions.
*/
...
public:
	...
	/*
	* Generates the conteacts to keep this link from being
	* viloated. This calss can onéy ever generate a single 
	* contact, so the pointer can be a pointer to a single
	* element, the limit parameter is assumed to be at least 1
	* (0 isnt valid), and the return values is 0 if the
	* cables wasnt extended, or 1 if a contact was needed.
	* 
	* NB: this method is declared in the same way (as pure
	* virtual) in the parent class, but is replicated here for 
	* documentation purposes.
	*/
	virtual void addContact(ParticleContact* contact, unsigned limit) = 0;
};
...
/*
* Rods link a pair of the particles, generating a contact if they
...
public:
	/*
	* Fills the given contact structure with the contatc nedded
	* to keep rod from extending or compressing.
	*/
	virtual unsigned addContact(ParticleContact* contact, unsigned limit) const;
