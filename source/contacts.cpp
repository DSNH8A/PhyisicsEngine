...
void Contact::calculateContactBasis()
{
	...
	//Check whether the Z-axis is nearer to X-axis or Y-axis
	...
	{
		//Scaling factor to ensure the results are normaized.
	...
	//Make the matrix from theese vectors.
...
Vector3 Contact::calculateFrictionlessImpulse(Matrix3* inverseInertiaTensor)
{
    ...
    if (body[1])
    {
        ...
        //Amount of impuslive torque geberated from a unit  of impulse.
        Vector3 deltaVelWorld = relativeContactPosition[1] % contactNormal;

        //Change un angular velocity for unit of impulsive torque.
        deltaVelWorld = inverseInertiaTensor[1].transform(deltaVelWorld);

        //Velocity of a point due to ony rotation.
...
void Contact::calculateDesiredDeltaVelocity(real duration)
{
    ...
    Vector3 scaledContact = contactNormal * duration;

    if (body[0]->getAwake())
    {
        velocityFromAcc = body[0]->getLastFrameAcceleration() * scaledContact;
    }

    if (body[1] && body[1]->getAwake())
    {
        velocityFromAcc -= body[1]->getLastFrameAcceleration() * scaledContact;
    }
...
void Contact::applyPositionChange(Vector3 linearChange[2], Vector3 angularChange[2], real penetration)
{
    ...
    for (unsigned i = 0; i < 2; i++) if (body[i])
    {
        ...
        //Calculate the objects approximated size so larger objects can have
        //angular movement.
...
void Contact::calculateInternals(real duration)
{
    ...
    //Store the raltive psition of the contact relative to each body.
    ...
    //Find the raltive velocity of the bodies at the contact point.
...
//    Vector3 contactVelocity = contactToWorld.transformTranspose(velocity);
//
//    //Calculate the amount of velocity tha is due to forces without
//    //reactions.
//    Vector3 accVelocity = thisBody->getLastFrameAcceleration() * duration;
//
//    //And return it.
...
void ContactResolver::adjustPositions(Contact* c, unsigned numContacts, real duration)
{
    ...
    //Iteratively resolve interpentrations in order of severity.
            ...
            for (unsigned b = 0; b < 2; b++)
            {
                if (c[i].body[b])
                {
                    ...
                    for (unsigned d = 0; d < 2; d++)
                    {
                        ...
                        {
                            deltaPosition = linearChange[4] + angularChange[d].vectorProduct(c[i].relativeContactPosition[b]);

                            //The sign of the change is positive if we are
                            //dealing with second body in a contact,
                            //and negative otherwise (because we are
...
{
    //make sure we ahve something to do.
    if (numContacts == 0)
    {
        return;
    }
    ...
    //Resolve the interpenetration problems iwth the contacts.
    adjustPositions(contacts, numContacts, duration);

    //Resolve velocity problems with the contacts.
...
void ContactResolver::prepareContacts(Contact* contacts, unsigned numContacts, real duration)
{
    ...
    for(Contact* contact=contacts; contact < lastContact; contact++)
    {
       ...
       // contact->claculateInternals(duration);
